...
def create_html_header(post, imagerelation):
	...
	html_header = """
			<div class="header">
				<span class="imagecontainer"><img class="header_image_%s" src="%s"/></span>
...
""" % (
		...
		post.date,
		post.text,
		"<!--<span class='l'>%d</span><span class='c'>%d</span><span class='s'>%d</span>-->" % (post.likes, len(post.comments), post.shares)
	)
	return html_header
...
""" % (
		"visible" if answer.author == "_kundendienst" else "hidden",
		assemble_path(
			RESPATH,
			"people",
...
def create():
	...
	try:
		...
		print "cannot create output file:",e
		sys.exit(1)
	else:
		try:
			f_html.write(create_html_post(post_object))
		except BaseException as e:
	...
	try:
		...
		print "Cannot write resource files: %s" % str(e)
		sys.exit(1)
	sys.exit(0)

if __name__ == "__main__":
	if len (sys.argv) < 2:
		print "usage: cc.py -hct [source] [destination]"
		sys.exit(1)
